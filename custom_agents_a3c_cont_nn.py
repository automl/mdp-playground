...
state_space_dims = [2]#, 10, 12, 14] # [2**i for i in range(1,6)]
action_space_dims = [2]#2, 4, 8, 16] # [2**i for i in range(1,6)]
delays = [0]# + [2**i for i in range(4)]
sequence_lengths = [10]#, 2, 3, 4]#i for i in range(1,4)]
# reward_densities = [0.25] # np.linspace(0.0, 1.0, num=5)
# make_reward_dense = [True, False]
# terminal_state_densities = [0.25] # np.linspace(0.1, 1.0, num=5)
algorithms = ["A3C"]
...
print('# Algorithm, state_space_dim, action_space_dim, delay, sequence_length')
print(algorithms, state_space_dims, action_space_dims, delays, sequence_lengths)
...
fout.write('# Algorithm, state_space_dim, action_space_dim, delay, sequence_length, num_layers, layer_width, dummy_seed,\n')
fout.close()
...
#     print("++++++++", aaaa, stats)
    ...
    state_space_dim = info["result"]["config"]["env_config"]["state_space_dim"]
    action_space_dim = info["result"]["config"]["env_config"]["action_space_dim"]
...
#    hack_filename = '/home/rajanr/custom-gym-env/' + SLURM_ARRAY_TASK_ID + '.csv'
    ...
    fout.write(str(algorithm) + ' ' + str(state_space_dim) +
               ' ' + str(action_space_dim) + ' ' + str(delay) + ' ' + str(sequence_length) + ' ')
               # + str(reward_density) + ' ' + str(terminal_state_density) + ' ')
               # Writes every iteration, would slow things down. #hack
    fout.write(str(num_layers) + ' ' + str(layer_width) + ' ' + str(dummy_seed) + ' ' + str(timesteps_total) + ' ' + str(episode_reward_mean) +
...
#    print("###on_episode_end", info["episode"].agent_rewards)
    ...
    if "dummy_eval" in info["env"].get_unwrapped()[0].config:
        print("###on_episode_end info", info["env"].get_unwrapped()[0].config["dummy_eval"], info["episode"].total_reward, info["episode"].length) #, info["episode"]._agent_reward_history)
...
for algorithm in algorithms: #TODO each one has different config_spaces
    for state_space_dim in state_space_dims:
        for action_space_dim in action_space_dims:
            for delay in delays:
