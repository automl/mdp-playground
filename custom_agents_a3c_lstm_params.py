...
fout.write('# Algorithm, state_space_size, action_space_size, delay, sequence_length, reward_density, '
               'terminal_state_density, lstm_cell_size, lstm_use_prev_action_reward, dummy_seed,\n')
fout.close()
...
#     print("++++++++", aaaa, stats)
    ...
    lstm_cell_size = info["result"]["config"]["model"]["lstm_cell_size"]
    lstm_use_prev_action_reward = int(info["result"]["config"]["model"]["lstm_use_prev_action_reward"])
...
#    hack_filename = '/home/rajanr/custom-gym-env/' + SLURM_ARRAY_TASK_ID + '.csv'
    ...
    fout.write(str(lstm_cell_size) + ' ' + str(lstm_use_prev_action_reward) + ' ' + str(dummy_seed) + ' ' + str(timesteps_total) + ' ' + str(episode_reward_mean) +
