...
def generate_plots(exp_name, dir_name, show_plots=False, options=""):
    print(
        ...
        + ": "
        + exp_name
        + " with the following addnl. options: "
        + options
    )
    ...
    (
        ...
        exp_name,
        load_eval=load_eval,
        normalise_episodic_reward=normalise_episodic_reward,
    )
    ...
    if "plot_2d" in options:
        mdpp_analysis.plot_2d_heatmap(
            train_aucs, save_fig, show_plots=show_plots, common_y_scale=common_y_scale
        )
    ...
    if "learn_curves" in options:
        mdpp_analysis.plot_learning_curves(
            train_curves, save_fig, show_plots=show_plots, common_y_scale=common_y_scale
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        "-f",
        type=str,
        help="Expt. identifiers and names listed in a YAML file, i.e., dir_name: exp_name",
    )
    ...
    parser.add_argument(
        ...
        "-e",
        type=str,
        help="expt name, corresponds to the names of the CSV stats files and the <config>.py file used for the expt.",
    )
    ...
    if args.exp_file is not None:
        ...
        for dir_name in yaml_dict:
            if len(yaml_dict[dir_name]) > 1:
                print(
                    "More than 1 expt. for the same expt_id:",
                    dir_name,
                    ". The expts.:",
                    yaml_dict[dir_name],
                )
            for j in range(len(yaml_dict[dir_name])):
                ...
                options = (
                    " ".join(yaml_dict[dir_name][j].split(" ")[1:])
                    if " " in yaml_dict[dir_name][j]
                    else ""
                )
                # if 'learn_curves' in options:
        ...
        dict_args["options"] = (
            " ".join(dict_args["dir_name"].split(" ")[1:])
            if " " in dict_args["dir_name"]
            else ""
        )
        # print(dict_args)
