...
class MDPP_Analysis:
    ...
    def load_data(
        ...
        self.config_names = config_names[
            1:
        ]  # ; begins at 1 to ignore training iteration num. ['Delay', 'Sequence Length', 'Reward Density', 'Terminal State Density', 'P Noise', 'R Noise', 'dummy_seed']
        config_counts = []
        dims_values = []
        ...
        for i in range(
            1, len(config_names) - num_metrics
        ):  # hardcoded 3 for no. of stats written
        ...
        config_counts.append(
            num_metrics
        )  # hardcoded number of training stats that were recorded
        ...
        for i in range(stats_pd.shape[0] - 1):
            if (
                stats_pd.iloc[i, -num_metrics] > stats_pd.iloc[i + 1, -num_metrics]
            ):  # hardcoded: -num_metrics column always HAS to be no. of timesteps for the current run
        ...
        stats_reshaped = stats_end_of_training.iloc[
            :, -num_metrics:
        ]  # hardcoded # last vals are timesteps_total, episode_reward_mean, episode_len_mean
        ...
        if load_eval:
            ...
            if hack_indices[0] == 0:  # hack
                hack_indices = hack_indices[
                    1:
                ]  # hardcoded removes the 1st hack_index which is at position 0 so that hack_indices_10 below doesn't begin with a -10; apparently Ray seems to have changed logging for evaluation (using on_episode_end) from 0.7.3 to 0.9.0
                ray_0_9_0 = True
            else:
                ray_0_9_0 = False
            ...
            final_eval_metrics_ = mean_data_eval[
                final_rows_for_a_config, :
            ]  # 1st column is episode reward, 2nd is episode length in original _eval.csv file, here it's 2nd and 3rd after prepending timesteps_total column above.
        ...
        dims_varied = []
        for i in range(
            len(self.config_counts) - 1
        ):  # -1 is added to ignore #seeds as dimensions of difficulty #hack
            if self.config_counts[i] > 1:
                ...
                if "state_space_dim" == self.config_names[i]:
                    ...
                    x_tick_labels_[-1] = [
                        str(j - 0) for j in self.dims_values[i]
                    ]  # 2 is #hardcoded default value of length of relevant_indices config
                    ...
                    x_tick_labels_[-1] = [
                        hack_process_label(j) for j in self.dims_values[i]
                    ]  # j is a str like "(0.8,1.25)"
                elif (
                    self.config_names[i] in ["sequence_length", "delay"]
                    and normalise_episodic_reward == True
                ):
                    print(
                        "Re-scaling episodic reward by (1 / sequence length) or (100/(100-delay)) to normalise maximal episodic reward across sequence lengths / delays."
                    )
                    for j in range(self.config_counts[i]):
                        ...
                        ind_ = (
                            (slice(None),) * (i)
                            + (j,)
                            + (slice(None),) * (n_dims_ - i - 1)
                            + (1,)
                        )
                        # print(self.config_names[i], self.dims_values[i][j], "train_aucs BEFORE", train_aucs[ind_])
                        # print("ind_", ind_)
                        mult = self.normaliser_episodic_reward(
                            self.config_names[i], self.dims_values[i][j]
                        )
                        # print("MULT", mult)
    ...
    def plot_1d_dimensions(
        ...
        if bonferroni:  # Apply bonferroni corrections
            ...
            n_configs = np.prod(
                self.config_counts[:-1]
            )  # Total no. of diff configs that were ran. Ignore last dimension num_seeds
            # print(n_configs, self.config_counts)
            if n_configs > 1:
                alpha /= comb(
                    n_configs, 2
                )  # Number of comparisons being made in the 1-D plots is between any 2 configs at once so n choose 2
            else:
                warnings.warn(
                    "alpha value unchanged with Bonferroni corrections as n_configs is <= 1, i.e., no comparisons being made."
                )
                print("alpha:", alpha)
        mean_data_ = np.mean(
            stats_data[..., metric_num], axis=-1
        )  # the slice sub-selects the metric written in position metric_num from the "last axis of diff. metrics that were written" and then the axis of #seeds becomes axis=-1 ( before slice it was -2).
        ...
        if err_bar == "std":
            ...
            to_plot_bars_ = to_plot_bars_[
                np.newaxis, :
            ]  # Make it 3-D like other to_plot_bars_ below
        elif err_bar == "t_dist":
            perc = 1 - alpha / 2
        ...
        if save_fig:
            plt.savefig(
                self.stats_file.split("/")[-1]
                + ("_train" if train else "_eval")
                + "_final_reward_"
                + self.axis_labels[0].replace(" ", "_")
                + "_"
                + str(self.metric_names[metric_num])
                + "_1d.pdf",
                dpi=300,
                bbox_inches="tight",
            )
    ...
    def plot_2d_heatmap(
        ...
        label_ = (
            "Reward"
            if "reward" in self.metric_names[metric_num]
            else self.metric_names[metric_num]
        )
        ...
        plt.figure(figsize=(fig_width, fig_height))
        plt.imshow(
            np.atleast_2d(to_plot_), cmap=cmap, interpolation="none", vmin=0, vmax=vmax
    ...
    def plot_learning_curves(
        ...
        if len(self.dims_varied) > 1:
            ...
            if self.normalise_episodic_reward:
                if self.config_names[self.dims_varied[1]] in [
                    "sequence_length",
                    "delay",
                ]:
        ...
        for i in range(len(self.final_rows_for_a_config)):
            ...
            if i == 0:
                to_plot_ = stats_data[
                    0 : self.final_rows_for_a_config[i] + 1, metric_num
                ]
            ...
            if self.normalise_episodic_reward:
                if len(self.dims_varied) > 1:
                    if rescale_list[0][0] == True:
                        mult = self.normaliser_episodic_reward(
                            rescale_list[0][1],
                            self.dims_values[self.dims_varied[0]][i_index],
                        )
                        to_plot_ *= mult
                    if rescale_list[1][0] == True:
                        mult = self.normaliser_episodic_reward(
                            rescale_list[1][1],
                            self.dims_values[self.dims_varied[1]][j_index],
                        )
                        # print(to_plot_)
                    ...
                    if rescale_list[0][0] == True:
                        mult = self.normaliser_episodic_reward(
                            rescale_list[0][1],
                            self.dims_values[self.dims_varied[0]][j_index],
                        )
                        to_plot_ *= mult
            ...
            if i % nseeds_ == nseeds_ - 1:  # 10 is num. of seeds
                ...
                if len(self.dims_varied) > 1:
                    title_1st_dim = (
                        self.process_axis_labels(self.config_names[self.dims_varied[0]])
                        + " "
                        + str(self.dims_values[self.dims_varied[0]][i_index])
                    )
                    title_2nd_dim = (
                        self.process_axis_labels(self.config_names[self.dims_varied[1]])
                        + " "
                        + str(self.dims_values[self.dims_varied[1]][j_index])
                    )
                else:
                    title_1st_dim = (
                        self.process_axis_labels(self.config_names[self.dims_varied[0]])
                        + " "
                        + str(self.dims_values[self.dims_varied[0]][j_index])
                    )
                    title_2nd_dim = ""
                ax[i_index][j_index].set_title(
                    title_1st_dim
                    + ((", " + title_2nd_dim) if title_2nd_dim != "" else "")
                )
