...
fout.write('# Algorithm, state_space_size, action_space_size, delay, sequence_length, reward_density, '
               'terminal_state_density, vf_loss_coeff, entropy_coeff, dummy_seed,\n')
fout.close()
...
#     print("++++++++", aaaa, stats)
    ...
    vf_loss_coeff = info["result"]["config"]["vf_loss_coeff"]
    entropy_coeff = info["result"]["config"]["entropy_coeff"]
...
#    hack_filename = '/home/rajanr/custom-gym-env/' + SLURM_ARRAY_TASK_ID + '.csv'
    ...
    fout.write(str(vf_loss_coeff) + ' ' + str(entropy_coeff) + ' ' + str(dummy_seed) + ' ' + str(timesteps_total) + ' ' + str(episode_reward_mean) +
                ...
                for sequence_length in sequence_lengths:
                    for reward_density in reward_densities:
                        for terminal_state_density in terminal_state_densities:
                            for vf_loss_coeff in vf_loss_coeffs:
