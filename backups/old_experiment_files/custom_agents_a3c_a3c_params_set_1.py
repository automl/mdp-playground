...
fout.write('# Algorithm, state_space_size, action_space_size, delay, sequence_length, reward_density, '
               'terminal_state_density, lambda, grad_clip, dummy_seed,\n')
fout.close()
...
#     print("++++++++", aaaa, stats)
    ...
    lambda_ = info["result"]["config"]["lambda"]
    grad_clip = info["result"]["config"]["grad_clip"]
...
#    hack_filename = '/home/rajanr/custom-gym-env/' + SLURM_ARRAY_TASK_ID + '.csv'
    ...
    fout.write(str(lambda_) + ' ' + str(grad_clip) + ' ' + str(dummy_seed) + ' ' + str(timesteps_total) + ' ' + str(episode_reward_mean) +
