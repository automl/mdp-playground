...
#    else if fcnet_activation == "relu":
    ...
    ddqn = info["result"]["config"]["double_q"]
    ddqn = int(bool(ddqn))
    ...
    fout.write('# Algorithm, state_space_size, action_space_size, delay, sequence_length, reward_density, '
               'terminal_state_density, num_layers, layer_width, fcnet_activation, double_dqn\n' + str(algorithm) + ' ' + str(state_space_size) +
    ...
    fout.write(str(num_layers) + ' ' + str(layer_width) + ' ' + str(fcnet_activation) + ' ' + str(ddqn) + ' ' + str(timesteps_total) + ' ' + str(episode_reward_mean) +
