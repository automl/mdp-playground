...
class TestGymEnvWrapper(unittest.TestCase):
    ...
    def test_cont_irr_features(self):
        ...
        assert ob.shape[0] == hc3w.env.observation_space.shape[0] + hc3w.irr_toy_env.observation_space.shape[0], "Shapes of base cont. env, irrelevant toy env: " + str(hc3w.env.observation_space.shape) + str(hc3w.irr_toy_env.observation_space.shape)
        total_reward = 0.0

        # check that random elements are correctly set in the observation and action spaces of component env's
        assert hc3w.env.observation_space.low[3] == hc3w.observation_space.low[3], "Concatenation of observation spaces of components envs to external observation spaces was not done correctly. Compared values: " + str(hc3w.env.observation_space.low[3]) + ", " + str(hc3w.observation_space.low[3])
        assert hc3w.irr_toy_env.action_space.high[1] == hc3w.action_space.high[hc3w.env.action_space.shape[0] + 1], "Concatenation of action spaces of components envs to external action spaces was not done correctly. Compared values: " + str(hc3w.irr_toy_env.action_space.high[1]) + ", " + str(hc3w.action_space.high[hc3w.env.action_space.shape[0] + 1])
        ...
        for i in range(200):
            ...
            if i == 0:
                ...
                np.testing.assert_allclose(next_state[hc3w.env.observation_space.shape[0]:], [-2.95129723, 0.05893834], rtol=1e-05, err_msg="Mismatch for irr. toy envs' state in step " + str(i) + ".")
            total_reward += reward
        print("total_reward:", total_reward)
        hc3w.reset()
